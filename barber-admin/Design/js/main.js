...
$('.cancel_appointment_button').click(function()
{
    ...
    $.ajax({
        url: "index.php?route=ajax/appointments",
        type: "POST",
        ...
        success: function (data) 
        {
            ...
            swal("Hủy lịch","Đã hủy lịch hẹn thành công!", "success").then((value) => 
            {
...
$('#add_category_bttn').click(function()
{
    ...
    else
    {
        $.ajax(
        {
            url:"index.php?route=ajax/service-categories",
            method:"POST",
            ...
            success: function (data) 
            {
                ...
                {
                    swal("Cảnh báo",data['message'], "warning").then((value) => {});
                }
                if(data['alert'] == "Success")
                {
                    $('#add_new_category').modal('hide');
                    swal("Thành công",data['message'], "success").then((value) => 
                    {
                        window.location.replace("index.php?route=service-categories");
                    });
                }
...
$('.delete_service_bttn').click(function()
{
    ...
    $.ajax(
    {
        url:"index.php?route=ajax/services",
        method:"POST",
        data:{service_id:service_id,do:do_},
        success: function (data) 
        {
            swal("Xóa dịch vụ","Đã xóa dịch vụ thành công!", "success").then((value) => {
                window.location.replace("index.php?route=services");
            });     
        },
        error: function(xhr, status, error) 
        {
 ...
 $('.delete_employee_bttn').click(function()
{
    ...
    $.ajax(
    {
        url:"index.php?route=ajax/employees",
        method:"POST",
        data:{employee_id:employee_id,do:do_},
        success: function (data) 
        {
            swal("Xóa nhân viên","Đã xóa nhân viên thành công!", "success").then((value) => {
                window.location.replace("index.php?route=employees");
            });     
        },
        error: function(xhr, status, error) 
        {
...
/* Thanh tìm kiếm toàn cục (header) — đã gỡ */