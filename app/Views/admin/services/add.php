    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card shadow-sm" style="border-radius:12px; overflow:hidden; border:none">
                <!-- Header -->
                <div style="background:linear-gradient(135deg, #e74a3b, #f06048); padding:24px 28px">
                    <div class="d-flex align-items-center">
                        <div style="width:52px; height:52px; border-radius:50%; background:rgba(255,255,255,0.2); display:flex; align-items:center; justify-content:center; margin-right:16px; flex-shrink:0">
                            <i class="fas fa-cut fa-lg" style="color:#fff"></i>
                        </div>
                        <div>
                            <h5 class="mb-0" style="color:#fff; font-weight:600">Thông tin dịch vụ</h5>
                            <div style="color:rgba(255,255,255,0.75); font-size:0.85rem">Điền đầy đủ để thêm dịch vụ mới</div>
                        </div>
                    </div>
                </div>
                    ...
                    <form method="POST" action="<?= admin_route('services', ['do' => 'Add']) ?>">
                                    ...
                                    <select class="form-control mt-1" name="service_category"
                                        ...
                                        <?php foreach ($categories as $category): ?>
                                            <option value="<?= (int)$category['ma_danh_muc'] ?>"
                                                <?= (($old['service_category'] ?? '') == $category['ma_danh_muc']) ? 'selected' : '' ?>>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control mt-1 <?= !empty($errors['service_duration']) ? 'is-invalid' : '' ?>"
                                           ...
                                           value="<?= htmlspecialchars($old['service_duration'] ?? '') ?>"
                                           placeholder="VD: 30"
                                           style="border-radius:8px; height:42px; border:1px solid #e2e8f0">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" inputmode="numeric" class="form-control money-input mt-1 <?= !empty($errors['service_price']) ? 'is-invalid' : '' ?>"
                                           ...
                                           value="<?= htmlspecialchars($old['service_price'] ?? '') ?>"
                                           placeholder="VD: 300.000"
                                           style="border-radius:8px; height:42px; border:1px solid #e2e8f0">
                                    <small class="text-muted">Nhập số tiền, hệ thống tự thêm dấu phân cách.</small>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control mt-1" name="service_description" rows="3"
                                      ...
                                      style="border-radius:8px; border:1px solid #e2e8f0; resize:none"><?= htmlspecialchars($old['service_description'] ?? '') ?></textarea>
                        ...
                        <div class="d-flex justify-content-end" style="gap:10px; padding-top:16px; border-top:1px solid #e2e8f0; margin-top:8px">
                            ...
                            <button type="submit" name="add_new_service" class="btn btn-danger px-4" style="border-radius:8px">
